...
/** Creates instances of [[RowWriter]] objects for the given row type `T`.
  ...
  * which cannot be simply mapped by a [[com.khulnasoft.spark.connector.mapper.ColumnMapper ColumnMapper]]. */
...
/** Provides a low-priority implicit `RowWriterFactory` able to write objects of any class for which
  * a [[com.khulnasoft.spark.connector.mapper.ColumnMapper ColumnMapper]] is defined. */
...
/** Provides an implicit `RowWriterFactory` for saving [[com.khulnasoft.spark.connector.CassandraRow CassandraRow]] objects. */
