...
import com.khulnasoft.spark.connector.cql.{CassandraConnector, ColumnDef}
...
import com.khulnasoft.spark.connector.util.{CountingIterator, MergeJoinIterator, NameTools, schemaFromCassandra}
...
class CassandraMergeJoinRDD[L,R](
  ...
  override def compute(split: Partition, context: TaskContext): Iterator[(Seq[L], Seq[R])] = {
    ...
    type T = t forSome { type t <: com.khulnasoft.spark.connector.rdd.partitioner.dht.Token[V] }
