...
import com.khulnasoft.bdp.util.ScalaJavaUtil._
import com.khulnasoft.oss.driver.api.core.CqlSession
import com.khulnasoft.oss.driver.api.core.cql.{BoundStatement, Row}
...
import com.khulnasoft.spark.connector.cql.{CassandraConnector, ColumnDef, Schema}
import com.khulnasoft.spark.connector.rdd.CassandraCoGroupedRDD._
import com.khulnasoft.spark.connector.rdd.partitioner.{CassandraPartition, CqlTokenRange, NodeAddresses}
import com.khulnasoft.spark.connector.rdd.reader.{PrefetchingResultSetIterator, RowReader}
import com.khulnasoft.spark.connector.types.ColumnType
import com.khulnasoft.spark.connector.util.Quote._
import com.khulnasoft.spark.connector.util.{CountingIterator, MultiMergeJoinIterator, NameTools}
...
class CassandraCoGroupedRDD[T](
  ...
  override def compute(split: Partition, context: TaskContext): Iterator[Seq[Seq[T]]] = {
    ...
    type K = t forSome { type t <: com.khulnasoft.spark.connector.rdd.partitioner.dht.Token[V] }
