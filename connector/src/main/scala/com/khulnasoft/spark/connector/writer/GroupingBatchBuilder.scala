...
import com.khulnasoft.spark.connector.util.PriorityHashMap
...
private[connector] class GroupingBatchBuilder[T](
    ...
    maxBatches: Int,
    data: Iterator[T])
  extends GroupingBatchBuilderBase[T](
    boundStatementBuilder,
    batchStatementBuilder,
    batchKeyGenerator,
    batchSize,
    maxBatches)
  with Iterator[RichStatement] {
...
private[connector] class GroupingBatchBuilderBase[T](
  boundStatementBuilder: BoundStatementBuilder[T],
  batchStatementBuilder: BatchStatementBuilder,
  batchKeyGenerator: RichBoundStatementWrapper => Any,
  batchSize: BatchSize,
  ...
  protected[this] val batchMap = new PriorityHashMap[Any, Batch](maxBatches)
  ...
  protected def processStatement(batchKey: Any, boundStatement: RichBoundStatementWrapper): Option[RichStatement] = {
  ...
  /** Adds the given statement to the batch if possible; If there is no enough capacity in the batch,
    ...
    * to it. */
  protected def updateBatchInMap(batchKey: Any, batch: Batch, newStatement: RichBoundStatementWrapper): Option[RichStatement] = {
  ...
  /** Adds a new batch to the buffer and adds the given statement to it. Returns a statement which had
    * to be dequeued. */
  protected def addBatchToMap(batchKey: Any, newStatement: RichBoundStatementWrapper): Option[RichStatement] = {
  ...
  @inline
  final protected def createStmtAndReleaseBatch(batch: Batch): RichStatement = {
  ...
  @inline
  protected def replaceBatch(batch: Batch, newStatement: RichBoundStatementWrapper, newBatchKey: Any): RichStatement = {
  ...
  @inline
  def batchRecord(record: T): Option[RichStatement] = {
    val wrapper = boundStatementBuilder.bind(record)
    val key = batchKeyGenerator(wrapper)
    processStatement(key, wrapper)
  }

  def finish(): Seq[RichStatement] = {
    for (batch <- batchMap.values.filter(_.statements.nonEmpty)) yield createStmtAndReleaseBatch(batch)
  }
}
