...
import com.khulnasoft.spark.connector.types.TypeAdapters.{ValueByNameAdapter, ValuesSeqAdapter}
import com.khulnasoft.spark.connector.types.{NullableTypeConverter, TypeConverter}
import com.khulnasoft.spark.connector.util.ConfigCheck.ConnectorConfigurationException
import com.khulnasoft.spark.connector.util.DriverUtil.toAddress
import com.khulnasoft.spark.connector.util.{Logging, SerialShutdownHooks}
...
class CassandraConnector(val conf: CassandraConnectorConf)
  ...
  import com.khulnasoft.spark.connector.cql.CassandraConnector._
...
object CassandraConnector extends Logging {
  ...
  SerialShutdownHooks.add("Clearing session cache for C* connector", 200)(() => {
    val classLoaderCheckName = "com.khulnasoft.spark.connector.util.ClassLoaderCheck"
    try {
