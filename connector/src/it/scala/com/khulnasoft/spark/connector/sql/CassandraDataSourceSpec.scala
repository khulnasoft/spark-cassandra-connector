...
import com.khulnasoft.spark.connector.cql.{CassandraConnector, TableDef}
import com.khulnasoft.spark.connector.datasource.CassandraInJoin
import com.khulnasoft.spark.connector.{SparkCassandraITFlatSpecBase, _}
...
class CassandraDataSourceSpec extends SparkCassandraITFlatSpecBase with DefaultCluster with BeforeAndAfterEach {
  ...
  it should "apply user custom predicates which erase basic pushdowns" in withConfig(
      ...
      "com.khulnasoft.spark.connector.sql.PushdownNothing") {
  ...
  it should "apply user custom predicate rules in the order they are specified" in withConfig(
      CassandraSourceRelation.AdditionalCassandraPushDownRulesParam.name,
      "com.khulnasoft.spark.connector.sql.PushdownNothing,com.khulnasoft.spark.connector.sql.PushdownEverything,com.khulnasoft.spark.connector.sql.PushdownEqualsOnly") {
    ...
    val df = spark
      ...
      .options(Map(
        ...
        "table" -> "test1",
        "pushdown" -> "true",
        CassandraSourceRelation.AdditionalCassandraPushDownRulesParam.name -> "com.khulnasoft.spark.connector.sql.PushdownNothing,com.khulnasoft.spark.connector.sql.PushdownEverything,com.khulnasoft.spark.connector.sql.PushdownEqualsOnly"))
      .load()
  ...
  it should "pass through local conf properties" in withConfig(
      CassandraSourceRelation.AdditionalCassandraPushDownRulesParam.name,
      "com.khulnasoft.spark.connector.sql.PushdownUsesConf") {
