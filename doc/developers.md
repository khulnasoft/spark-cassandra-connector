...
Let's assume that `khulnasoft` is `git@github.com:khulnasoft/spark-cassandra-connector.git` remote 
and origin is your personal clone.
```shell
$ git remote -v
khulnasoft	git@github.com:khulnasoft/spark-cassandra-connector.git (fetch)
khulnasoft	git@github.com:khulnasoft/spark-cassandra-connector.git (push)
...
```
...
```shell
git fetch khulnasoft
git checkout -b SPARKC-9999-b2.5 khulnasoft/b2.5
# do the work, commit
git push origin SPARKC-9999-b2.5
 
# Forward merge on the next version:
git checkout -b SPARKC-9999-b3.0 khulnasoft/b3.0
...
# Forward merge on the next version:
git checkout -b SPARKC-9999-b3.1 khulnasoft/b3.1
...
# Forward merge on the next version:
git checkout -b SPARKC-9999-master khulnasoft/master
...
#### test-support
CCM Wrapper code. Much of this code is based on the Khulnasoft Java Driver's test code. 
...
Our CI Build runs through the Khulnasoft Infrastructure and tests on all the builds
...
```bash
./bin/spark-shell --repositories file:/Users/yourUser/.m2/repository --packages com.khulnasoft.spark:spark-cassandra-connector_2.10:1.6.0-14-gcfca49e
```
