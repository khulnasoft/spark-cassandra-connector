...
```scala
libraryDependencies += "com.khulnasoft.spark" %% "spark-cassandra-connector" % "1.6.0" 
```
...
A longer example (with source code) of the Connector Java API is on the KhulnaSoft tech blog:
[Accessing Cassandra from Spark in Java](https://www.khulnasoft.com/dev/blog/accessing-cassandra-from-spark-in-java).
