...
import com.khulnasoft.oss.driver.api.core.data.{TupleValue => DriverTupleValue, UdtValue => DriverUDTValue}
import com.khulnasoft.spark.connector.types.TypeConverter.StringConverter
import com.khulnasoft.spark.connector.util.ByteBufferUtil
...
object GettableData {
  ...
  /** Deserializes given field from the KhulnaSoft Java Driver `Row` into appropriate Java type.
  ...
  /** Deserializes given field from the KhulnaSoft Java Driver `Row` into appropriate Java type by using predefined codec
    * If the field is null, returns null (not Scala Option). */
